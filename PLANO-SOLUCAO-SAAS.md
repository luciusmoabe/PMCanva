...
- Atividade da organização ("Atividade" na sidebar, secao GESTAO — antes um item de menu morto com um "ponto de nao lido" decorativo que nunca refletia nada real, removido): feed com os eventos de auditoria de todos os projetos da organizacao, nao so do projeto ativo, cada item mostrando de qual projeto se trata. Reaproveita a mesma `audit_events` e a mesma RLS que ja liberava isso — so precisou de uma consulta com join em `projects` para trazer o nome.
